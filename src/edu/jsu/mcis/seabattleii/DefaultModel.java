...
public final class DefaultModel extends AbstractModel {
    ...
    public void setPlayer1SquareHit(HashMap<String, Integer> s) {
        ...
        if (square.isHit()) {
            firePropertyChange(DefaultController.PLAYER1_ALREADY_HIT, s, square.isHit());
            return;
        } else {
            square.hit();
        }
        ...
        if (square instanceof ShipSquare) {
            ...
            if (ship.isSunk()) { // If sunk, decrement fleet count and fire a Sink property change
                ...
                if (player1.getCount() == 0) {
                    firePropertyChange(DefaultController.PLAYER1_FLEET_SUNK, s, player1.getCount());
                }
            }
    ...
    public void setPlayer2SquareHit(HashMap<String, Integer> s) {
        ...
        if (square.isHit()) {
            firePropertyChange(DefaultController.PLAYER2_ALREADY_HIT, s, square.isHit());
            return;
        } else {
            square.hit();
        }
        ...
        if (square instanceof ShipSquare) {
            ...
            if (ship.isSunk()) { // If sunk, decrement fleet count and fire a Sink property change
                ...
                if (player2.getCount() == 0) {
                    firePropertyChange(DefaultController.PLAYER2_FLEET_SUNK, s, player2.getCount());
                }
            }
            else { // If not sunk, fire a Hit property change
