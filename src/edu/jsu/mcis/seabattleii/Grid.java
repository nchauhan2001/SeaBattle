    ...
    public void deployShip(Ship s) { // INSERT YOUR CODE HERE
        ...
        while ( !empty ) {
            ...
            /* If horizontal, check for empty space in next column */
                if (alignment == Ship.HORIZONTAL) {
                    if (grid.get(row).get(col + i) instanceof EmptySquare) { ++emptyCount; }
                }

            /* If vertical, check for empty space in next row */        
            
                else {
                    if (grid.get(row + i).get(col) instanceof EmptySquare) { ++emptyCount; }
                }
        ...
        for (int i = 0; i < shipSize; ++i) {
            ...
            /* If horizontal, place Ship in next column */

            if (alignment == Ship.HORIZONTAL) {
                grid.get(row).set(col + i, new ShipSquare(s, i));
            }

            /* If vertical, place Ship in next row */
